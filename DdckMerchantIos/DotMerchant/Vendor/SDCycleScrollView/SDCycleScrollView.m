...
- (void)scrollToIndex:(int)targetIndex
{
    if (targetIndex >= _totalItemsCount) {
        if (self.infiniteLoop) {
            ...
            [_mainView setContentOffset:CGPointMake(self.boworrWidth-((Screen_Width-self.boworrWidth)-_cellSpace)/2.0+_cellSpace/2.0, _mainView.contentOffset.y) animated:NO];
        }
        return;
    }
...
- (void)layoutSubviews
{
    ...
    _flowLayout.itemSize = CGSizeMake(self.boworrWidth, self.sd_height);
...
{
    if (self.imagePathsGroup.count == 0 || self.imagePathsGroup == nil){
        return 0;
    }else {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.imageView.layer.cornerRadius = self.imageViewRoundedCorner;
